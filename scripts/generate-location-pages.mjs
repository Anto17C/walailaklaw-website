...
const locations = [
  {
    ...
    description: 'English-speaking legal services in Chonburi for international businesses, foreign residents and families, led by Walailak Law Firm with local support when useful.',
    lead: 'Walailak-led legal support for international businesses, foreign residents and families across Chonburi province—from commercial and industrial matters to property, disputes and urgent representation.',
    contextTitle: 'A connected legal market with very different client needs',
    context: 'Chonburi combines major industrial estates, international supply chains, Laem Chabang Port, established residential communities and Pattaya’s tourism economy. A matter may involve provincial courts and authorities, an industrial estate, a Land Office or specialist local attendance. Kae can lead and handle the matter directly, adding suitable local counsel only when the work benefits from it.',
    ...
    focus: 'Chonburi is a broad provincial market. Kae determines whether to handle the full matter herself or include counsel from Pattaya, Si Racha or Laem Chabang for a particular court, authority or local task.',
  ...
  },
  {
    ...
    description: 'Walailak-led legal services in Si Racha for international companies, executives and foreign residents, with local support when the matter requires it.',
    ...
    context: 'Si Racha sits between major industrial estates, residential communities and Laem Chabang’s logistics economy. Clients commonly need help that crosses company, employment, immigration, contract and property issues. Kae leads the matter in English and may handle it entirely herself or add suitable local counsel for specific location-dependent work.',
  ...
  },
  {
    ...
    description: 'Walailak-led legal support in Laem Chabang for logistics, shipping, industrial and trading businesses, with local capability available when required.',
    lead: 'Walailak-led legal assistance for logistics operators, importers, exporters, manufacturers, contractors and international businesses connected to Laem Chabang.',
    ...
    focus: 'Kae may handle the full engagement directly. If a dispute, court, authority or operational issue calls for additional local capability, she selects and manages that support while remaining lead counsel and the client’s central contact.',
  ...
  },
  {
    ...
    lead: 'Independent, Walailak-led legal guidance for foreign property buyers, international residents and businesses in Phuket, with local capability available when it adds value.',
  ...
  },
  {
    ...
    description: 'Walailak-led English-speaking legal services in Chiang Mai for foreign residents, international families, property matters and businesses.',
    lead: 'Clear, Walailak-led legal support for foreign residents, international families, property clients and businesses in Chiang Mai through one trusted point of contact.',
    contextTitle: 'Practical legal support for long-term residents and international families',
    context: 'Chiang Mai attracts long-term foreign residents, international couples, retirees, entrepreneurs and small businesses. Their matters often combine Thai documentation with overseas assets, family relationships or remote decision-makers. Kae can handle the engagement remotely or travel as appropriate, bringing in Chiang Mai counsel only when local attendance or added capability benefits the client.',
    ...
    focus: 'Remote consultations and document review can begin before a client travels. Kae decides whether to handle every stage herself, travel when appropriate or add a Chiang Mai lawyer for a defined local role while she continues to lead the matter.',
...
function locationPage(x) {
  ...
  return `${head(x.title,x.description,`/locations/${x.slug}`)}${tracking}${header}<main><section class="hero hero-sm" style="--hero-img-mobile:url('/images/${x.image}');background-image:linear-gradient(90deg,rgba(20,17,13,.97) 0%,rgba(20,17,13,.91) 38%,rgba(20,17,13,.35) 100%),url('/images/${x.image}');"><div class="container"><div class="hero-inner"><div class="breadcrumb"><a href="/">Home</a> / <a href="/locations">Locations</a> / ${x.name}</div><span class="eyebrow">WALAILAK-LED · LOCAL CAPABILITY</span><h1>English-Speaking Lawyers in ${x.name}</h1><p class="lead">${x.lead}</p></div></div></section><section class="section"><div class="container"><div class="two-col"><div><span class="eyebrow light">Legal support in ${x.name}</span><h2 style="margin:14px 0 16px;">${x.contextTitle}</h2><p class="text-secondary location-copy">${x.context}</p></div><div class="location-panel"><h3>Who we assist</h3><ul class="location-checks">${clients}</ul></div></div></div></section><section class="section on-tint"><div class="container"><div class="section-header"><span class="eyebrow light">Relevant services</span><h2>Legal matters we handle in ${x.name}</h2><p>Kae determines whether to handle the matter directly or include local counsel for a defined role, based on the facts, jurisdiction and practical needs.</p></div><div class="services-grid location-services">${cards}</div></div></section><section class="section"><div class="container"><div class="two-col"><div><span class="eyebrow light">How our flexible model works</span><h2 style="margin:14px 0 16px;">One lead lawyer, flexible delivery</h2><p class="text-secondary location-copy">${x.focus}</p><ol class="location-steps"><li><strong>Initial review:</strong> Kae assesses the facts, urgency and conflicts.</li><li><strong>Clear scope:</strong> You receive a clear plan explaining who will handle each part of the work and the fees.</li><li><strong>Walailak-led delivery:</strong> Kae leads the engagement, whether she handles it entirely herself or uses local counsel for a defined task.</li></ol></div><div>${contactRow}</div></div></div></section><section class="section-sm on-tint"><div class="container"><h2 style="font-size:20px;margin-bottom:16px;">Related locations</h2><div class="location-related">${related}</div></div></section></main>${footer}`;
}
...
const hub = `${head('Legal Services Across Thailand | Walailak Law Firm','Find Walailak Law Firm offices and Walailak-led legal support in Rayong, Pattaya, Chonburi, Si Racha, Laem Chabang, Phuket and Chiang Mai.','/locations')}${tracking}${header}<main><section class="hero hero-sm" style="--hero-img-mobile:url('/images/office-signage-hero.webp');background-image:linear-gradient(90deg,rgba(20,17,13,.97),rgba(20,17,13,.75)),url('/images/office-signage-hero.webp');"><div class="container"><div class="hero-inner"><div class="breadcrumb"><a href="/">Home</a> / Locations</div><span class="eyebrow">OFFICES · WALAILAK-LED · NATIONWIDE CAPABILITY</span><h1>Legal Services Across Thailand</h1><p class="lead">Meet us at our Rayong or Pattaya offices, or work directly with Kae in major Thai business, industrial and residential centres, with local counsel added when useful.</p></div></div></section><section class="section"><div class="container"><div class="section-header"><span class="eyebrow light">Where we work</span><h2>One trusted point of contact, backed by local capability</h2><p>These pages explain how service is delivered and which legal needs are most relevant in each market. A coverage location is not presented as a Walailak branch office.</p></div><div class="location-grid">${hubCards}</div></div></section><section class="section on-dark"><div class="container"><div class="two-col"><div><span class="eyebrow">THE WALAILAK MODEL</span><h2 style="color:#fff;margin:14px 0 16px;">Direct leadership with flexible delivery</h2><p class="location-copy" style="color:var(--text-on-dark-secondary);">Kae reviews the enquiry and decides the best way to handle it. She may manage every stage herself, work remotely or travel; local counsel is added only when attendance, specialist input or practical efficiency benefits the client.</p></div><div>${contactRow}</div></div></div></section></main>${footer}`;
